...
class CMAES(OOOptimizer):  # could also inherit from object
    ...
    def __init__(self, xstart, sigma,  # mandatory
                 ...
                 randn=random_normalvariate, bufflength=30):
        ...
        self.cov_buffer = collections.deque(maxlen=bufflength)
        for _ in range(bufflength):
            self.cov_buffer.append(DecomposingPositiveMatrix(N))
    ...
    def tell(self, arx, fitvals):
        ...
        cov_t = DecomposingPositiveMatrix(self.params.dimension)
        cov_t.multiply_with(0)
        cov_t.addouter(self.pc, par.c1/(par.c1 + par.cmu * sum(par.weights)))  # cov_t = c1 * pc * pc^T, so-called rank-one update
        for k, wk in enumerate(par.weights):  # so-called rank-mu update
            ...
            cov_t.addouter(minus(arx[k], xold),  # cov_t += wk * cmu * dx * dx^T
                            wk * par.cmu/(par.c1 + par.cmu * sum(par.weights)) / self.sigma**2)
        self.C.multiply_with(0)
        self.C.add_with(cov_t)
        for cov in self.cov_buffer:
            self.C.add_with(cov)
        self.C.multiply_with(1/(len(self.cov_buffer)+1))
        self.cov_buffer.append(cov_t)
...
class SquareMatrix(list):  # inheritance from numpy.ndarray is not recommended
    ...
    def add_with(self, square_matrix):
        for i in range(len(self)):
            for j in range(len(self[i])):
                self[i][j] += square_matrix[i][j]
        return self
